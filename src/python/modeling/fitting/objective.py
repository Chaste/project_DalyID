...
class MeanEuclidean(MinimizedObjective):
	...
	def __call__(cls,singleData,repeatData,args={}):
		...
		for j in range(nrepeats):
			...
			# Allow for normalization by experimental values
			if args['std'] == "exp":
				std = dict([(name,repeatData[name][j]) for name in repeatData])

			for i,name in enumerate(outputs):	
				# If each data are ndarray, the euclidean distance will be summed over all dimensions
				simdat = numpy.array(singleData[name])
				expdat = numpy.array(repeatData[name][j])
				euc = ((simdat-expdat)/std[name])**2
