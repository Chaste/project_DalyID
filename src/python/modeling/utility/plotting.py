...
def PlotHeatMap(distribution,dest='heatmap',include=None,bins=50,plotRange=None,
	...
	for i in range(len(include)):
		for j in range(i+1,len(include)):			
			...
			# Manually specify 5 x-ticks to hedge against label overlapping
			xmin,xmax = xyRange[0][0], xyRange[0][1]
			xscale = xmax-xmin
			ax.set_xticks([xmin,xmin+xscale*.25,xmin+xscale*.5,xmin+xscale*.75,xmax])

			if separateFigs:
				fig.colorbar(im)
				# Last argument prevents clipping of axis labels for large font size
				plt.savefig(dest+"_"+px+"_"+py+".eps",format="eps",bbox_inches="tight")
			else:
				ims.append(im)
	...
	if not separateFigs:
		...
		# Last argument prevents clipping of axis labels for large font size
		plt.savefig(dest+".eps",format="eps",bbox_inches="tight")
