...
''' Used to represent priors on rate parameters in the hERG model '''
class LogUniform(Distribution):
	''' Initiate an RV that is an exponentiation of a uniform random variable '''
	def __init__(self,lb,ub,base=None):
		self.lb = float(min(lb,ub))
		self.ub = float(max(lb,ub))
		self.base = base

		if base != None:
			assert isinstance(base,float) or isinstance(base,int), "Log uniform requires numerical base"
	
	def __repr__(self):
		if self.base != None:
			return "Log_"+str(self.base)+"_Uniform("+str(self.lb)+","+str(self.ub)+")"
		else:
			return "Ln_Uniform("+str(self.lb)+","+str(self.ub)+")"

	def draw(self):
		if self.base == None:
			return numpy.exp(rand.uniform(self.lb,self.ub))
		else:
			return self.base**(rand.uniform(self.lb,self.ub))
	def pdf(self,val):
		if self.base == None:
			if val < numpy.exp(self.lb) or val > numpy.exp(self.ub):
				return 0
			else:
				return 1.0/(self.ub-self.lb) * (1.0/val - self.lb)
		else:
			if val < self.base**self.lb or val > self.base**self.ub:
				return 0
			else:
				return 1.0/(self.ub-self.lb) * (1.0/(val*numpy.log(self.base)) - self.lb)
	def cdf(self,val):
		if self.base == None:
			if val < numpy.exp(self.lb):
				return 0
			elif val > numpy.exp(self.ub):
				return 1
			else:
				return (numpy.log(val)-self.lb)/(self.ub-self.lb)
		else:
			if val < self.base**lb:
				return 0
			elif val > self.base**ub:
				return 1
			else:
				return (numpy.log(val)/numpy.log(self.base) - self.lb)/(self.ub-self.lb)
	def min(self):
		if self.base == None:
			return numpy.exp(self.lb)
		else:
			return self.base**self.lb
	def max(self):
		if self.base == None:
			return numpy.exp(self.ub)
		else:
			return self.base**self.ub
